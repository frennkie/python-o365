...
from .recipient import Recipient, HandleRecipientsMixin
...
class Message(ApiComponent, AttachableMixin, HandleRecipientsMixin):
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        # ToDo(frennkie) not sure whether this is needed
        self.message_type = cloud_data.get('@odata.type', None)
    ...
    def get_event(self):
        """ Returns the related meeting event from a meeting message """

        url = self.build_url(
            self._endpoints.get('get_message').format(id=self.object_id))

        query = Query(protocol=self.protocol).expand('microsoft.graph.eventMessage/event')
        ...
        event = Event(parent=self, **{self._cloud_data_key: data.get('event')})
        if not event:
            return None
